...
function callbackFn(fn, r){
	if(fn!=undefined){
		if (! $.isFunction(fn)) fn = eval('(' + fn + ')');
		fn(r);
	}
}
...
window.alert = function(txt, confirmFn, time){
	...
	var _confirmFn = confirmFn||function(){};
	var _time = time||'';
	if(_time == null || _time == ''){
		_time = 1500;
	}
	mylayerFn.msg(txt, {time: _time}, _confirmFn);
};
...
window.confirm = function(txt, confirmFn, cancelFn){
	mylayerFn.confirm({
        ...
        icon: 3,
        yesFn: function(){
        	callbackFn(confirmFn, true)
        },
        cancelFn: function(){
        	callbackFn(cancelFn, false)
        }
    });
};
...
function formParamSerizlize(obj){
	var data = '';
	$(obj).each(function(){
		data = data + $(this).val() + ',';
	});
