<#macro uploadFileListFtl relationId relationType="" paramName="fileInfos" divId="fileDiv" btnTxt="上传文件" fileNumLimit=0 fileTypeLimit="">
			...
			$.get('/file','relationId=${relationId}&relationType=${relationType}',function(data) {
				if (data != null) {
					$.each(data,function(i, tag) {
						...
						$li.addClass('success');
						$li.find('.mis-pbar').remove();
						$li.attr('fileId', this.id).show();
	...
	<div class="m-pbMod-udload">
        <a class="mis-opt-upbtn mis-inverse-btn picker"><i class="mis-upload-ico"></i>${btnTxt}</a>
    </div>
    <ul class="mis-sfile-lst fileList " style="width:800px;">
